...
class InputHandle:
    def __init__(self, input_param):
        self.paths = input_param['paths'] #contains the path to the datafile/folder (this case the npz file)
        self.num_paths = len(input_param['paths']) #len of path array
        self.name = input_param['name'] #naam dataset
        self.input_data_type = input_param.get('input_data_type', 'float32')
        self.output_data_type = input_param.get('output_data_type', 'float32')
        self.minibatch_size = input_param['minibatch_size']
        self.is_output_sequence = input_param['is_output_sequence'] #boolean
        self.data = {}
        self.indices = {}
    ...
    def load(self):
        dat_1 = np.load(self.paths[0]) #load npz file
        print("\t\tDATA PRINT")
        print(dat_1)
    ...
    def total(self):
        return self.data['index'].shape[1]
    ...
    def begin(self, do_shuffle = True):
        ...
        self.current_input_length = max(self.data['index'][0, ind, 1] for ind
                                        in self.current_batch_indices)
        self.current_output_length = max(self.data['index'][1, ind, 1] for ind
                                         in self.current_batch_indices)
    ...
    def input_batch(self):
        ...
        for i in range(self.current_batch_size):
            ...
            begin = self.data['index'][0, batch_ind, 0]
            end = self.data['index'][0, batch_ind, 0] + \
                    self.data['index'][0, batch_ind, 1]
            data_slice = self.data['data'][begin:end, :, :, :]
    ...
    def output_batch(self):
        ...
        for i in range(self.current_batch_size):
            batch_ind = self.current_batch_indices[i]
            begin = self.data['index'][1, batch_ind, 0]
            end = self.data['index'][1, batch_ind, 0] + \
                    self.data['index'][1, batch_ind, 1]
            if self.is_output_sequence:
